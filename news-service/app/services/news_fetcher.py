...
async def fetch_newsapi(req: FeedRequest) -> List[NormalizedArticle]:
    ...
    async with httpx.AsyncClient(timeout=REQUEST_TIMEOUT, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}) as client:
        try:
...
async def _fetch_rss(url: str, source_name: str, tickers: List[str]) -> List[NormalizedArticle]:
    # 1. robust headers so Google/Yahoo think we are a real Chrome browser
    headers = {
    ...
    # 2. follow_redirects=True is CRITICAL for Google News
    async with httpx.AsyncClient(timeout=REQUEST_TIMEOUT, follow_redirects=True, headers=headers) as client:
        try:
            resp = await client.get(url)
            resp.raise_for_status()
        except Exception as e:
            # 3. Print the error so we can actually see it in the terminal!
            print(f"⚠️ RSS Fetch Error for {source_name} ({url}): {e}")
            return []
